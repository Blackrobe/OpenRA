...
namespace OpenRA
{
	...
	public static class Sync
	{
		...
		// Per-thread reentrancy counter. Under Stage B (DecoupledRendering) the sim runs on its own thread while
		...
		public static T RunUnsynced<T>(bool checkSyncHash, World world, Func<T> fn)
		{
			...
			// Stage B (DecoupledRendering): while the sim runs on its own thread the world's sync hash changes
