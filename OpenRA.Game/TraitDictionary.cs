...
namespace OpenRA
{
	...
	sealed class TraitDictionary
	{
		...
		sealed class TraitContainer<T> : ITraitContainer
		{
			...
			// Stage B (DecoupledRendering): actors are added/removed on the SIM thread (spawn/death during the
			...
			{
				// Stage B: the sim thread can structurally mutate actors/traits (Add/RemoveActor) while the MAIN
			...
			{
				// Stage B: callers include the UNLOCKED render Draw (IRenderAboveWorld / IRenderShroud), which can
