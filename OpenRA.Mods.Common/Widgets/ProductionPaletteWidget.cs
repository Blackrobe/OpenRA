...
namespace OpenRA.Mods.Common.Widgets
{
	...
	public class ProductionPaletteWidget : Widget
	{
		...
		// Stage B (DecoupledRendering): Draw must not enumerate live sim-mutated queue state (BuildableItems,
		...
		// Stage B (DecoupledRendering): cache freshness across threads. See RefreshCachedState.
		...
		{
			// Stage B (DecoupledRendering): also refresh from the render frame, not just the logic Tick. The logic
		...
		{
			// Stage B (DecoupledRendering): read production state only when the sim thread is not mid-tick, so the
		...
		{
			// Stage B (DecoupledRendering): the click/hotkey handlers below read live queue state (AllQueued,
