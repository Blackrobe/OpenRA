...
namespace OpenRA.Mods.Common.Widgets
{
	...
	public class CommandBarLogic : ChromeLogic
	{
		...
		public CommandBarLogic(Widget widget, World world, Dictionary<string, MiniYaml> logicArgs)
		{
			...
			if (deployButton != null)
			{
				...
				deployButton.IsDisabled = () =>
				{
					...
					// Stage B (DecoupledRendering): CanIssueDeployOrder dispatches a live trait on a selected actor
		...
		{
			// Stage B (DecoupledRendering): this enumerates live Selection actors and their traits (TraitsImplementing
		...
		{
			// Stage B (DecoupledRendering): run a one-shot command-bar callback under the blocking world lock so its
		...
		{
			// Stage B (DecoupledRendering): one-shot order issue - take the blocking world lock so the selection
		...
		{
			// Stage B (DecoupledRendering): one-shot deploy issue - blocking world lock (see above).
			Game.EnterWorldReadLock();
			try
			{
				UpdateStateIfNecessary();
