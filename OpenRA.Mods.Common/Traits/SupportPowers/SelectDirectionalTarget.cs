...
namespace OpenRA.Mods.Common.Traits
{
	public class SelectDirectionalTarget : IOrderGenerator
	{
		...
		// E-09 (Stage B): bumped each time a directional target locks the cursor, so a deferred Deactivate reset can
		...
		void IOrderGenerator.Deactivate()
		{
			if (activated)
			{
				// Stage B (E-09): Deactivate is reachable from IOrderGenerator.Tick on the SIM thread (CancelInputMode
				...
				{
					// E-09: capture the cursor-lock generation now; a NEWER directional mode may activate (and re-Lock
