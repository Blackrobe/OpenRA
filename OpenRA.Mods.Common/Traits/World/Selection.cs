...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class Selection : ISelection, INotifyCreated, INotifyOwnerChanged, ITick, IGameSaveTraitData
	{
		...
		// Stage B (DecoupledRendering): the sim thread prunes the selection every tick (ITick.Tick RemoveWhere:
		...
		{
			// Stage B (DecoupledRendering): the SIM-thread ITick.Tick prune is the only off-main caller, and its
			...
			// Harden the deferred (sim-prune) run (codex/rev r6/r7 re-opens):
			//  - E-02: the callbacks read LIVE world state (order-generator SelectionChanged; INotifySelection, e.g.
			...
			//  - E-03: re-read the CURRENT selection at execution rather than replaying the prune-time `snapshot`, so a
